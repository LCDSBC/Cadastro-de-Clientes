...
export async function updateSaleStatus(
  saleId: string,
  status: Sale["status"],
): Promise<{ sale: Sale; integrationMessages: string[] } | null> {
  const locals = loadLocal();
  ...
  const { applySaleStatusSideEffects } = await import(
    "@/lib/sale-integrations"
  );
  const integration = await applySaleStatusSideEffects(
    updated,
    previousStatus,
    status,
  );

  return { sale: updated, integrationMessages: integration.messages };
}
