...
export interface SalesGoalReport {
  goal: number;
  achieved: number;
  pct: number;
  remaining: number;
...
export function buildSalespersonReport(
  sales: Sale[],
...
export function buildDelinquencyReport(
  accounts: FinancialAccount[],
): DelinquencyReport[] {
  ...
  return accounts
    ...
    .map((a) => {
      ...
      return {
        client_name: a.client_name ?? a.description,
        amount: a.amount,
        days_overdue: days,
        due_date: a.due_date,
      };
...
export function buildSalesGoalReport(
  sales: Sale[],
  goal: number,
  period: ReportPeriod = "month",
): SalesGoalReport {
  const achieved = sales
    .filter(
      (s) => s.status !== "cancelado" && inPeriod(s.created_at, period),
    )
