...
export default function RelatoriosPage() {
  ...
  const [delinquency, setDelinquency] = useState<DelinquencyReport[]>([]);
  const [goal, setGoal] = useState<SalesGoalReport | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      ([
        ...
        setSalespeople(
          buildSalespersonReport(sales, prefs.commission_percent ?? 3, period),
        );
        setDelinquency(buildDelinquencyReport(accounts));
        setGoal(
          buildSalesGoalReport(sales, prefs.sales_goal_monthly ?? 50000, period),
        );
        setLoading(false);
      },
    );
    ...
    <AppShell>
      ...
      {loading || !reports ? (
        ...
        <div className="space-y-8">
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <Card>
              <CardHeader><CardTitle>Meta de vendas</CardTitle></CardHeader>
              <CardContent className="space-y-2 text-sm">
                {goal && (
                  <>
                    <div className="flex justify-between">
                      <span>Meta</span>
                      <span className="font-medium">{formatCurrency(goal.goal)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Realizado</span>
                      <span className="font-medium text-emerald-700">{formatCurrency(goal.achieved)}</span>
                    </div>
                    <div className="h-2 rounded-full bg-slate-100">
                      <div className="h-full rounded-full bg-primary-500" style={{ width: `${Math.min(100, goal.pct)}%` }} />
                    </div>
                    <p className="text-xs text-slate-500">{goal.pct.toFixed(1)}% · Faltam {formatCurrency(goal.remaining)}</p>
                  </>
                )}
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                {salespeople.length ? salespeople.map((s) => (
                  <div key={s.name} className="flex justify-between border-b py-1">
                    <span>{s.name} ({s.count})</span>
                    <span className="font-medium">{formatCurrency(s.commission)}</span>
                  </div>
                )) : <p className="text-slate-500">Sem vendas com vendedor.</p>}
              </CardContent>
            </Card>
            <Card>
              <CardHeader><CardTitle>Inadimplência</CardTitle></CardHeader>
              <CardContent className="space-y-2 text-sm">
                {delinquency.length ? delinquency.slice(0, 5).map((d, i) => (
                  <div key={i} className="flex justify-between border-b py-1">
                    <span className="truncate">{d.client_name}</span>
                    <span className="text-red-600">{formatCurrency(d.amount)} · {d.days_overdue}d</span>
                  </div>
                )) : <p className="text-slate-500">Nenhum título vencido.</p>}
              </CardContent>
            </Card>
          </div>
        </div>
