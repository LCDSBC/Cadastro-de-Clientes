...
import type { Sale, SaleItem, Client, Product } from "@/lib/types";
import {
  loadSales,
  saveSale,
  updateSaleStatus,
...
export default function VendasPage() {
  ...
  useEffect(() => {
    Promise.all([loadSales(), loadClients(), loadProducts()]).then(
      ([{ sales: salesData }, { clients: clientsData }, { products: productsData }]) => {
        setSales(salesData);
  ...
  const handleAdvanceStatus = async (sale: Sale) => {
    ...
    const updated = await updateSaleStatus(sale.id, nextStatus);
    if (updated) {
      setSales((prev) =>
  ...
  const handleCancel = async (sale: Sale) => {
    ...
    const updated = await updateSaleStatus(sale.id, "cancelado");
    if (updated) {
      setSales((prev) =>
        prev.map((s) => (s.id === updated.id ? updated : s)),
      );
